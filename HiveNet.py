...
# DEF
apidomain = "HiveNet.top/dashboard/"

# MOTD 
motd = "Made By NexusLove"
...
HiveNet = '''{
  "embed_title": "HiveNet | Selfbot",
  "embed_color": "#00165B",
  "embed_thumbnail": "https://HiveNet.top/dashboard/assets/img/favicon.png",
  "embed_footer": "HiveNet on top",
  "embed_footer_icon": "https://HiveNet.top/dashboard/assets/img/favicon.png",
  "embed_url": "https://HiveNet.top/"
}
'''

if not os.path.exists('scripts/'): os.makedirs('scripts/');
if not os.path.exists('themes/'): os.makedirs('themes/');
if not os.path.isfile('favicon.ico'): open('favicon.ico', 'wb').write(requests.get(f'https://HiveNet.top/dashboard/download/favicon.ico', allow_redirects=True).content);  

def showtoastnotifier(description):
    if os.name == "nt":
        ToastNotifier().show_toast("HiveNet Notification",description,icon_path="favicon.ico",duration=5,threaded=True)

if not os.path.exists('./themes/HiveNet.json'):
    with open("./themes/HiveNet.json", "w") as f:
        f.write(HiveNet)
    
if os.path.exists('./themes/HiveNet.json'):
    with open("./themes/HiveNet.json", "r") as jsonFile:
        data = json.load(jsonFile)

if not os.path.exists('./scripts/example.py'):
    f = open("./scripts/example.py", "w")
    f.write('''@HiveNet.command()
...
@HiveNet.listen()
async def on_ready():
        showtoastnotifier("HiveNet is ready!")
...
def print_e(txt):
    ...
    # print(Colorate.Vertical(Colors.blue_to_cyan, Center.XCenter(text)))
    print(Colorate.Vertical(Colors.blue_to_cyan, text=f"[{time} | HiveNet] {txt}"))
...
async def on_server_ban(guild):
    if json.load(open('config.json'))['server_ban_notification']:
        ...
        notification.notify(
        app_name = "HiveNet Selfbot",
        title = 'HiveNet Selfbot - Server Ban',
        message = f'You have been banned from {guild.name}',
        app_icon = 'icon.ico',
        timeout = 10
    )
    webhook = discord.Webhook.from_url(json.load(open('config.json'))['serverban_webhook'], adapter=discord.AsyncWebhookAdapter(HiveNet))
    embed = discord.Embed(
...
async def on_relationship_remove(relationship):
    if json.load( open('config.json'))['relationship_notification']:
        if isinstance(relationship.type, discord.RelationshipType.outgoing_request):
            ...
            notification.notify(
            app_name = "HiveNet Selfbot",
            title = 'HiveNet Selfbot - Relationship',
...
async def blowjob(ctx):
    ...
    try:
        ...
        with IO.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"HiveNet_blowjob.gif"))
    except:
        embed = discord.Embed()
        embed.set_footer(text= EMBEDFOOTER)
...
@HiveNet.command(aliases=["masschannels", "masschannel"])
...
@HiveNet.command(aliases=['changehypesquad'])
...
@HiveNet.command(aliases=["themes"])
...
@HiveNet.command(aliases=["settheme", "theme"])
...
@HiveNet.command(alises=["game"])
async def playing(ctx, *, message):
    await ctx.message.delete()
    game = discord.Game(
        name=message
    )
    await HiveNet.change_presence(activity=game)

@HiveNet.command(aliases=["watch"])
async def watching(ctx, *, message):
    await ctx.message.delete()
    await HiveNet.change_presence(
        activity=discord.Activity(
            type=discord.ActivityType.watching,
            name=message
        ))

@HiveNet.command(aliases=["stopstreaming", "stopstatus", "stoplistening", "stopplaying", "stopwatching"])
async def stopactivity(ctx):
    await ctx.message.delete()
    await HiveNet.change_presence(activity=None, status=discord.Status.dnd)



# MOD COMMAND

@HiveNet.command(aliases=['friendbackup', 'serverbackup',"backupfriends","backupservers","backupfull","fullbackup"])
async def backup(ctx,snipestatus=None):
    ...
    for frend in HiveNet.user.friends:
        try:
...
async def kick(ctx, member: discord.Member = None):
  ...
  embed.description = f"{member.mention} has been kicked by {HiveNet.user.name}"
  await member.kick()
  await send_message_in_mode(ctx, embed)
...
async def ban(ctx, member: discord.Member = None):
  if member is None:
   ...
   embed = discord.Embed(color=EMBEDCOLOR, timestamp=ctx.message.created_at)
  embed.description = f"{member.mention} has been banned by {HiveNet.user.name}"
  await member.ban()
  await send_message_in_mode(ctx, embed)

@HiveNet.command(aliases=["fakename"])
...
async def webhookspammer(ctx, arg1, *, arg2):
    while True:
        try:
            adata = requests.post(arg1, json={'embeds': [{"title": arg2, "description": "[HiveNet](https://HiveNet.xyz)", "footer": {"text": "HiveNet"}, "image": {"url": "https://de.meming.world/images/de/thumb/3/3f/Professional_Retard.jpg/300px-Professional_Retard.jpg"}, "fields": [{"name": arg2, "value": arg2}, {"name": arg2, "value": arg2}]}], 'username': 'Fucked with HiveNet'})
...
async def cloneserver(ctx):
    ...
    newguild = await HiveNet.create_guild(ctx.message.guild.name)
    ...
    async with aiohttp.ClientSession() as cs:
        async with cs.get(
            ...
            try:
                ...
                with IO.BytesIO(image) as file:
                    await ctx.send(
                        file=discord.File(file, f"HiveNet_tweet.png"))
            except:
...
@HiveNet.command(aliases=["ttweet"])
...
#This should work as an animated nickname command
...
keep_alive()
HiveNet.run(token, bot=False, reconnect=True)